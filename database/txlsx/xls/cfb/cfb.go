...
func (d *Document) load(rx io.Reader) error {
	var err error
	...
	if h.Signature != 0xe11ab1a1e011cfd0 {
		return errors.New("ole2: invalid format") // errors.New("ole2: invalid format")
	}
	...
	if fullAssertions {
		if h.ClassID[0] != 0 || h.ClassID[1] != 0 {
			return errors.New("ole2: invalid CLSID") //errors.New("ole2: invalid CLSID")
		}

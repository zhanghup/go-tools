...
func (b *WorkBook) Get(sheetName string) (Collection, error) {
	for _, s := range b.sheets {
		if s.Name == sheetName {
			...
			for rn := int(firstRow); rn <= int(lastRow); rn++ {
				for cn := int(firstCol); cn <= int(lastCol); cn++ {
					if rn == int(firstRow) && cn == int(firstCol) {
						...
						if rn == int(lastRow) {
							res.Put(rn, cn, EndRowMerged, 0)
						} else {
							res.Put(rn, cn, ContinueRowMerged, 0)
						}
					} else if cn == int(lastCol) {
						// first and last column are NOT the same
						res.Put(rn, cn, EndColumnMerged, 0)
					} else {
						res.Put(rn, cn, ContinueColumnMerged, 0)
					}
				}
			}
